%% SameCell_Identification.m
%%% OCTOBER 15, 2020 

function [Cells,Cells_Reverse] = SameCell_Identification(Cells, ...
    Cells_Reverse)
...
while ~SameCell
    ...
    for k = 1:length(INDEX)
        for m = 1:length(INDEX{k})
            if k ~= INDEX{k}(m)
                if length(CELLS(k).frame) == ...
                        length(CELLS(INDEX{k}(m)).frame)
                    if norm(CELLS(INDEX{k}(m)).Centroid - ...
                            CELLS(k).Centroid) <= 1
                        if k > LFor
