...
function [trackedOBJS, removedOBJS, idN] = updateCellsNonCycling(M1, trackedOBJS, removedOBJS, newOBJs, idN, FRAME)

    MAX_MOTHER_DAUGHTER_DISPACEMENT = 36;
    MAX_CIRCLE_DISPLACEMENT = 50;   %36
    N1 = size(M1,1);
    updatedINDEXES = [];
    toREADD = [];
    ...
    for k = 1:N1
        ...
        if length(INDEX) == 1
            ...
            trackedOBJS(k).MeanGreenFluo(POS) = NaN;
%             trackedOBJS(k).MinGreenFluo(POS) = NaN;
%             trackedOBJS(k).MaxGreenFluo(POS) = NaN;
        ...
        elseif length(INDEX) > 1
            ...
            for h = 1:length(INDEX)
                NORMS(h) = norm(trackedOBJS(k).Centroid(end,:) - newOBJs(INDEX(h)).Centroid);
            end
            ...
            trackedOBJS(k).Centroid(POS,:) = newOBJs(sortedINDEXES(1)).Centroid;
            trackedOBJS(k).NucleusIdxPixel{POS} = newOBJs(sortedINDEXES(1)).NucleusIdxPixel;
            trackedOBJS(k).MeanRedFluo(POS) = newOBJs(sortedINDEXES(1)).MeanRedFluo;
            trackedOBJS(k).frame(POS) = FRAME;
            trackedOBJS(k).Area(POS) = NaN;
            trackedOBJS(k).MajorAxis(POS) = NaN;
            trackedOBJS(k).MinorAxis(POS) = NaN;
...
%             trackedOBJS(k).MaxGreenFluo(POS) = NaN;
            ...
            for D = 2:length(sortedINDEXES)
                ...
                if POS == 1
                    ...
                    lOBJs.NucleusIdxPixel{1} = newOBJs(sortedINDEXES(D)).NucleusIdxPixel;
                    lOBJs.MeanRedFluo = newOBJs(sortedINDEXES(D)).MeanRedFluo;
                    lOBJs.Area = NaN;
                    lOBJs.MajorAxis = NaN;
                    lOBJs.MinorAxis = NaN;
                    lOBJs.Orientation = NaN;
                    lOBJs.MeanGreenFluo = NaN;
%                     lOBJs.MinGreenFluo = NaN;
%                     lOBJs.MaxGreenFluo = NaN;
                    ...
                    if norm([trackedOBJS(k).Centroid(end,1) trackedOBJS(k).Centroid(end,2)] - [trackedOBJS(S+1).Centroid(end,1) trackedOBJS(S+1).Centroid(end,2)]) < MAX_MOTHER_DAUGHTER_DISPACEMENT    % se la distanza tra la cellula da cui siamo partiti nello script e la cellula che stiamo considerando ora è minore della distanza madre - figlia impostata

                        trackedOBJS(S+1).parentLABEL = trackedOBJS(k).LABEL;
                        trackedOBJS(S+1).ROOT = trackedOBJS(k).ROOT;
                        trackedOBJS(S+1).LINEAGE = [trackedOBJS(k).LINEAGE, trackedOBJS(k).LABEL];
