%% computeCellsNonCycling.m
%%% OCTOBER 14, 2020

function [trackedOBJs] = computeCellsNonCycling(brightIMG, greenIMG, ...
    trackedOBJs)
    ...
    for k = 1:length(trackedOBJs)
        ...
        if ~isempty(L)
            ...
            tempIMG = imfilter(tempIMG,h);
            tempBW = ~niblack(tempIMG, [10 10]);
            ...
            statsBW = regionprops(finalBW,tempGREEN,'Centroid','Area',...
                'MajorAxisLength','MinorAxisLength','Orientation',...
                'BoundingBox','MeanIntensity','Image','PixelIdxList');
            ...
            for j = 1:length(statsBW)
                ...
                if ~isempty(nuc)
                    ...
                    if norm(statsBW(j).Centroid-statsNucleus.Centroid) ...
                            <= statsBW(j).MajorAxisLength/2
                        tempOBJs = [tempOBJs;statsBW(j)];
                        
                        if length(tempOBJs)==1
                            
                            trackedOBJs(k).Area(POS) = tempOBJs.Area;
                            trackedOBJs(k).MajorAxis(POS) = ...
                                tempOBJs.MajorAxisLength;
                            trackedOBJs(k).MinorAxis(POS) = ...
                                tempOBJs.MinorAxisLength;
                            trackedOBJs(k).Orientation(POS) = ...
                                tempOBJs.Orientation;
                            trackedOBJs(k).MeanGreenFluo(POS) = ...
                                tempOBJs.MeanIntensity;
                            trackedOBJs(k).BoundingBox(POS,:) = ...
                                tempOBJs.BoundingBox;
                            trackedOBJs(k).RelativeNucleus(POS,:) = ...
                                nuc.Centroid;
                        ...
                        elseif length(tempOBJs)==2
                            ...
                            tempMASK((tempOBJs(1).PixelList(row,2)+...
                                tempOBJs(2).PixelList(col,2))./2,...
                                (tempOBJs(1).PixelList(row,1)+...
                                tempOBJs(2).PixelList(col,1))./2) = 1;
                        
                            tempstats = regionprops(tempMASK,tempGREEN,...
                                'Area','MajorAxisLength',...
                                'MinorAxisLength','Orientation',...
                                'BoundingBox','MeanIntensity','Image');
                            temp = imcrop(tempNUCLEI,...
                                tempstats(j).BoundingBox);
                            ...
                            trackedOBJs(k).MajorAxis(POS) = ...
                                tempstats.MajorAxisLength;
                            trackedOBJs(k).MinorAxis(POS) = ...
                                tempstats.MinorAxisLength;
                            trackedOBJs(k).Orientation(POS) = ...
                                tempstats.Orientation;
                            trackedOBJs(k).MeanGreenFluo(POS) = ...
                                tempstats.MeanIntensity;
                            trackedOBJs(k).BoundingBox(POS,:) = ...
                                tempstats.BoundingBox;
                            trackedOBJs(k).RelativeNucleus(POS,:) = ...
                                nucpos.Centroid;
        ...
        if isnan(trackedOBJs(k).Area(end))
            ...
            if length(trackedOBJs(k).Area) >= 2
                ...
                rect = [abs(trackedOBJs(k).Centroid(POS,1)-Xdist),...
                    abs(trackedOBJs(k).Centroid(POS,2)-Ydist),BB(3)-1,...
                    BB(4)-1];
                ...
                if size(tempGREEN,1) < size(tempBW,1)
                    if rect(2) <= size(tempGREEN,1)-size(tempBW,1)
                        tempBW = tempBW(size(tempGREEN,1)-...
                            size(tempBW,1):end,:);
                    else
                ...
                if size(tempGREEN,2) < size(tempBW,2)
                    if rect(1) <= size(tempGREEN,2)-size(tempBW,2)
                        tempBW = tempBW(:,size(tempGREEN,2)-...
                            size(tempBW,2):end);                     
                    else
                ...
                if size(tempBW,1) < size(tempGREEN,1)
                    if rect(2) <= size(tempBW,1)-size(tempGREEN,1)
                        tempGREEN = tempGREEN(size(tempBW,1)-...
                            size(tempGREEN,1):end,:);
                    else
                ...
                if size(tempBW,2) < size(tempGREEN,2)
                    if rect(1) <= size(tempBW,2)-size(tempGREEN,2)
                        tempGREEN = tempGREEN(:,size(tempBW,2)-...
                            size(tempGREEN,2):end);                     
                    else
                ...
                statsBW = regionprops(tempBW,tempGREEN,'Area',...
                    'MajorAxisLength','MinorAxisLength','Orientation',...
                    'BoundingBox','MeanIntensity','Image');
                ...
                if ~isempty(statsBW)
                    ...
                    trackedOBJs(k).MeanGreenFluo(POS) = statsBW.MeanIntensity;
                    trackedOBJs(k).BoundingBox(POS,:) = ...
                        statsBW.BoundingBox;
                    trackedOBJs(k).RelativeNucleus(POS,:) = ...
                        trackedOBJs(k).RelativeNucleus(POS-1,:);
                    trackedOBJs(k).Mask{POS} = statsBW.Image;
                end
            
            else
                MASK = maskOBJS(greenIMG, ...
                    trackedOBJs(k).Centroid(POS,:), 3);
                statsBW = regionprops(MASK,greenIMG,'Area',...
                    'MajorAxisLength','MinorAxisLength','Orientation',...
                    'BoundingBox','MeanIntensity','Image');
        ...
        if (length(trackedOBJs(k).Area)>=2) && ...
                ((trackedOBJs(k).Area(end)<trackedOBJs(k).Area(end-1)/1.5) || (trackedOBJs(k).Area(end)>trackedOBJs(k).Area(end-1)*1.5))
            ...
            Ydist = trackedOBJs(k).RelativeNucleus(POS-1,2);
            rect = [abs(trackedOBJs(k).Centroid(POS,1)-Xdist),...
                abs(trackedOBJs(k).Centroid(POS,2)-Ydist),BB(3)-1,BB(4)-1];
