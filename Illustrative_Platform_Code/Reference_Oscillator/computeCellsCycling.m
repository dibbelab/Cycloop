%% computeCellsCycling.m
%%% OCTOBER 14, 2020

function [trackedOBJs] = computeCellsCycling(brightIMG, greenIMG, ...
    trackedOBJs)
    ...
    for k = 1:length(trackedOBJs)
        ...
        if ~isempty(L)
            ...
            tempIMG = imcrop(brightIMG,rect); % Cropping the filtered image (for improved results)
            ...
            statsBW = regionprops(finalBW,tempGREEN,'Centroid',...
                'Area','MajorAxisLength','MinorAxisLength',...
                'Orientation','BoundingBox','MaxIntensity','Image',...
                'PixelIdxList');
                ...
                if ~isempty(nuc)
                    ...
                    if norm(statsBW(j).Centroid-statsNucleus.Centroid) ...
                        ...
                        if length(tempOBJs)==1
                            ...
                            trackedOBJs(k).MaxGreenFluo(POS) = ...
                                tempOBJs.MaxIntensity;
                        ...
                        elseif length(tempOBJs)==2
                            ...
                            for l = 1:length(tempOBJs(1).PixelIdxList)
                                for m = 1:length(tempOBJs(2).PixelIdxList)
                                    dis(l,m) = norm(...
                                        tempOBJs(1).PixelList(l,:) - ...
                                        tempOBJs(2).PixelList(m,:));
                                end
                            end
                            ...
                            tempstats = regionprops(tempMASK,tempGREEN,...
                                ...
                                'BoundingBox','MaxIntensity','Image');
        ...
        if (length(trackedOBJs(k).Area)>=2) && ...
                ((trackedOBJs(k).Area(end)<trackedOBJs(k).Area(end-1)/1.5) || ...
                (trackedOBJs(k).Area(end)>trackedOBJs(k).Area(end-1)*1.5))
